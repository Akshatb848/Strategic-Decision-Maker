"""
POST /api/v1/analysis — trigger ASIS pipeline with SSE streaming.
    ...
    try:
        while not graph_task.done() or not sse_queue.empty():
            try:
                ...
                yield await format_sse(event_item["event"], event_item["data"])
        ...
        yield await format_sse("error", {"analysis_id": analysis_id, "message": str(exc)})
...
async def _persist_results(
    ...
    try:
        async with db.begin_nested():
            ...
            # Update agent run records
            metadata = state.get("metadata", {})
            token_usage = metadata.get("token_usage", {})
            for agent_name in ["orchestrator", "market_intelligence", "risk_assessment",
                                "financial_reasoning", "competitor_analysis", "synthesis"]:
